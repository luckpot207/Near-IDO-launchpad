...
import { useNearContext, useNearLogin, useProject } from '../hooks'
...
export default function ListCard({
  ...
  let icon = role === accountIdNear ? WalletIcon : SettingLightIcon;

  const handleWalletClick = () => {
    role === accountIdNear ? navigate(`/setting/${projectId}`) : navigate(`/listing/${projectId}`)
  }
      ...
      <VStack width='100%' spacing='24px'>
        ...
        >
          <Box width='100%'>
            ...
            <Text as='h2' fontSize='18px' textAlign='start' marginTop='10px'>{(totalTokens / decimals).toLocaleString()}</Text>
          </Box>
          <Flex width='100%' justifyContent='end' flexDirection='column'>
            <Flex margin='5px' justifyContent='end'>
