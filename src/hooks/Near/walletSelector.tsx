...
export class SelectorAccount extends Account {
    ...
    constructor(wallet: Wallet | null, config: NearAppConfig, accountId: string | null) {
    ...
    async refresh(selector: WalletSelector) {
        ...
        this.realAccountId =
            selector.store.getState().accounts.find((acc) => acc.active)?.accountId || null;
    }
    ...
    override async sendMoney(receiverId: string, amount: BN): Promise<FinalExecutionOutcome> {
        throw new Error("Not supported");
    }
    ...
    override async deleteAccount(beneficiaryId: string): Promise<FinalExecutionOutcome> {
        throw new Error("Not supported");
    }

    override async deployContract(data: Uint8Array): Promise<FinalExecutionOutcome> {
        throw new Error("Not supported");
    }

    override async functionCall(props: FunctionCallOptions): Promise<FinalExecutionOutcome>;
                ...
                actions: [
                    {
                        ...
                        params: {
                            ...
                            args: props.args,
                            gas: props.gas?.toString() || DEFAULT_FUNCTION_CALL_GAS.toString(),
    ...
    override async deleteKey(publicKey: string | PublicKey): Promise<FinalExecutionOutcome> {
        throw new Error("Not supported");
    }
    ...
    override async getAccountDetails(): Promise<{ authorizedApps: AccountAuthorizedApp[] }> {
        throw new Error("Not supported");
    }
...
export const setupSelector = async (config: NearAppConfig) => {
    ...
    const _modal = setupModal(_selector, { contractId: config.pegasusContractId });
...
export const refreshWalletConnection = async (
    ...
    const account = selector.store.getState().accounts.find((acc) => acc.active)?.accountId || null;
    const signOut = async () => {
        if (!isLoggedIn) {
            return;
        }
    ...
    const sign = async (message: string) => {
        ...
        if (wallet.id === "meteor-wallet") {
            ...
            const publicKey = `ed25519:${bs58.encode(Buffer.from(result.publicKey, "base64"))}`;
