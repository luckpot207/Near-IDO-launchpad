...
import { useColor, useNearLogin, useProjects, useActiveProject, useNearContext } from '../hooks';
...
export default function ProjectDashboard() {
  ...
  const getUserBalance = async (ftContractId: string) => {
    ...
    const balance = await ftContract.getFtBalanceOfOwnerFormatted(accountIdNear);
          ...
          {submitOpen ? (
              ...
              <Flex maxWidth='lg' flexDirection='column' marginX='auto'>
                ...
                >
                  ...
                  <Flex alignItems='center' flexDirection='column' justifyContent='center' marginY={4}>
                    ...
                    />
                    <Text as='h2' fontSize='16px' textAlign='center' >${userProjects[projectId].token_ticker} </Text>
                  </Flex>
                    ...
                    >
                      ...
                      <Flex justifyContent='end' flexDirection='column'>
                        <Flex margin='5px' justifyContent='end'>
                          <Text as='h2' fontSize='1vw' textAlign='end' marginTop='10px' width='max-content' color={color.main}>{userProjects[projectId].title}</Text>
                        </Flex >
                        <Flex justifyContent='end' margin='5px'>
                          <Image src={userProjects[projectId].logo} />
                          <Text as='h1' fontSize='16px' textAlign='end' marginLeft='15px' color={color.black}>{userProjects[projectId].token_ticker}</Text>
                        </Flex>
                      </Flex>
                    </Flex>
                    <Flex
                  ...
                  <Flex >
                    <Button
                      width={'100%'}
                      {...depositButtonStyle}
                      onClick={() => handleDeposit(userProjects[projectId].out_token_account_id, userProjects[projectId].project_id)}
                      isDisabled={!activate}
                    >
                      DEPOSIT
                    </Button>
            ...
            <>
              <Flex width={'100%'} justifyContent={'flex-end'}>
                <IconButton variant='ghost' aria-label='Prev project' icon={<PageLeftIcon />} onClick={() => handlePrev(userProjects.length - 1, projectId)} />
                <IconButton variant='ghost' aria-label='Next project' icon={<PageRightIcon />} onClick={() => handleNext(userProjects.length - 1, projectId)} />
              </Flex>
              <Flex
                width='100%'
                marginY='4'
              ...
              >
                <Flex justifyContent='space-between'>
                  <Flex flexDirection='column' width='65%'>
                    <LabelCard title='PROJECT / TOKEN NAME' value={userProjects[projectId].title} />
                    <LabelCard title='SUB TITLE' value={userProjects[projectId].sub_title} />
                    <LabelCard title='TOKEN ID' value={userProjects[projectId].out_token_account_id} />
                    <Flex justifyContent={'space-between'}>
                      <LabelCard title='TOKEN TICKER' value={userProjects[projectId].token_ticker} />
                      <LabelCard title='STARTING PRICE (USD)' value={(userProjects[projectId].starting_price / inTokenDecimals).toString()} />
                    </Flex>
                    <LabelCard title='E-MAIL' value={userProjects[projectId].email} />
                    <LabelCard title='TELEGRAM CONTACT' value={userProjects[projectId].telegram} />
                    <LabelCard title='CHOOSE TOKEN TICKER TO RECEIVE' value={userProjects[projectId].in_token_account_id} />
                    <LabelCard title='TOTAL DEPOSIT TOKEN AMOUNT(FOR LAUNCHPAD)' value={(userProjects[projectId].total_tokens / (10 ** decimals)).toString()} />
                    <LabelCard title='COINGECKO / COINMARKETCAP LINK (OPTIONAL)' value={userProjects[projectId].coingecko ?? ""} />
                    <LabelCard title='FACEBOOK (OPTIONAL)' value={userProjects[projectId].facebook ?? ""} />
                    <LabelCard title='INSTAGRAM (OPTIONAL)' value={userProjects[projectId].instagram ?? ""} />
                    <LabelCard title='TWITTER (OPTIONAL)' value={userProjects[projectId].twitter ?? ""} />
                  </Flex>
                  <Flex flexDirection='column' width='30%'>
                    ...
                    <LabelCard title='IDO START DATE & TIME' value={`${startTime.toLocaleDateString()} ${startTime.toLocaleTimeString()}`} />
                    <LabelCard title='IDO END DATE & TIME' value={`${endTime.toLocaleDateString()} ${endTime.toLocaleTimeString()}`} />
                  </Flex>
                </Flex>
                <Flex justifyContent='center' flexDirection='column'>
                  <LabelCard title='DESCRIPTION' value={userProjects[projectId].description} />
                ...
                <Flex justifyContent='end' marginTop='8'>
                  <Button width={40} position='relative' {...primaryButtonStyle} onClick={() => setSubmitOpen(true)} isDisabled={userProjects[projectId].is_activated}>DEPOSIT & TOKEN</Button>
                </Flex>
              </Flex>
            </>
          )}
