...
import { Box, Flex, Text, Image, Button, SkeletonCircle } from '@chakra-ui/react';
import { BiChevronLeft as ArrowLeftIcon } from 'react-icons/bi';
import { useColor, useProject, useBalance, useNearLogin, useNearContext, useWithdrawOutToken } from '../hooks';
...
export default function Detail() {
  ...
  const handleWithdraw = async () => {
    const res = await projectWithdrawOutToken({ projectId: Number(projectId), amount: null })
    console.log('res', res)
  }
  ...
  if (project.isLoading || project.isError) return (
    ...
    const cliffEndTime = project.value.end_time / TimeDivision + Number(project.value.cliff_period)
    const cliffRemainDays = new Date(project.value.cliff_period).getDate() - 1
    const cliffProgressValue = cliffStartTime > now ? 0 : 100 * (now - cliffStartTime) / project.value.cliff_period;
          ...
          <Flex width='100%' flexDirection={'column'} marginY={8}>
            ...
            <Flex flexDirection={'column'} marginY={2}>
              ...
              <Text fontSize='14px' textAlign='center' padding='2px 0px 0px 8px' color={color.green}>
                {project.value.title} IDO {ended ? 'has been' : 'will be'} completed as at {ShortMonthNames[endTime.getMonth()]} {endTime.getDate()}, {endTime.getFullYear()}
              </Text>
            </Flex>
          </Flex>
            ...
            <Flex justifyContent={'space-between'}>
              <Flex width={'45%'} flexDirection={'column'}>
                ...
                <Flex flexDirection={'column'} marginY={8} >
                  <Flex paddingX={4} marginY={2} alignItems='end' justifyContent={'space-between'}>
                    <Flex width={'20%'}>
                      <Text as='h5' fontSize='14px' fontWeight='500' fontFamily='DM Sans' textAlign='start' >
                        {new Date(cliffStartTime).getDate()} {ShortMonthNames[new Date(cliffStartTime).getMonth()]} {new Date(cliffStartTime).getFullYear()}
                      </Text>
                    </Flex>
                    <Flex width={'60%'} flexDirection={'column'} justifyContent={'center'} >
                    ...
                    <Flex width={'20%'} justifyContent={'end'}>
                      <Text as='h5' fontSize='14px' fontWeight='500' fontFamily='DM Sans' textAlign='end' >
                        {new Date(cliffEndTime).getDate()} {ShortMonthNames[new Date(cliffEndTime).getMonth()]} {new Date(cliffEndTime).getFullYear()}
                      </Text>
                    </Flex>
                  </Flex>
                  <Flex
                    ...
                    overflow='hidden'
                  >
                    <Box width={`${cliffProgressValue}%`} left='0' bgColor={'#76DEFF'} height='100%' zIndex={300} position='absolute'></Box>
                    <Flex position='absolute' zIndex={500} justifyContent='center' flexDirection={'column'} minWidth='100%'>
                      <Text fontSize='10px' textAlign='center' paddingTop='2' color={color.background}>TOKEN LOCK</Text>
                      <Text fontSize='20px' textAlign='center' paddingBottom='2' color={color.background}>
                        {cliffRemainDays}  {cliffRemainDays ? 'DAYS CLIFF PERIOD' : 'DAY CLIFF PERIOD'}
                      </Text>
                    </Flex>
                  </Flex>
                </Flex>
              ...
              <Flex width={'45%'} flexDirection={'column'}>
                ...
                <Flex flexDirection={'column'} marginY={8} >
                  ...
                  <Flex
                    ...
                    overflow='hidden'
                  >
                    <Box width={`${unlockProgressValue}%`} left='0' bgColor={color.lightGreen} height='100%' zIndex={300} position='absolute'></Box>
                    <Flex position='absolute' zIndex={500} justifyContent='center' flexDirection={'column'} minWidth='100%'>
                      <Text fontSize='10px' textAlign='center' paddingTop='2' color={color.green}>TOKEN UNLOCK</Text>
                      <Text fontSize='20px' textAlign='center' paddingBottom='2' color={color.green}>TOKEN DISTRIBUTION</Text>
                    </Flex>
                  </Flex>
                </Flex>
                <Flex justifyContent='end' >
                  <Button width={'100%'} {...secondaryButtonStyle} onClick={handleWithdraw} isDisabled={now < cliffEndTime}>WITHDRAW</Button>
                </Flex>
              </Flex>
            </Flex>
            <Flex
            ...
            >
              ...
              <Flex width={'100%'} justifyContent={'space-between'} marginY={2}>
                <InfoCard
                  ...
                  value={(project.value.total_tokens / decimals).toLocaleString()}
              ...
              <Flex width={'100%'} justifyContent={'space-between'} marginY={2}>
                <InfoCard
                  ...
                  value={(project.value.total_deposits / inTokenDecimals).toLocaleString()}
                ...
                <InfoCard
                  ...
                  value={(userDepositedBalance / inTokenDecimals).toLocaleString()}
                  tokenTitle={'Tether USD'}
                  tokenLogo={USDT}
                  tokenTicker={'USDT.e'}
                />
