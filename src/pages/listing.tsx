...
export default function Listing() {
  ...
  const handleDeposit = () => {
    ...
    projectDepositInToken({ accountId: accountIdNear, projectId: Number(projectId), ftContract: usdtContract, amount: depositBalance })
  }

  const handleWithdraw = async () => {
    if (withdrawBalance <= 0) return;
    const res = await projectWithdrawInToken({ projectId: Number(projectId), amount: withdrawBalance })
    console.log('res', res)
  }
  ...
  if (project.isLoading || project.isError) return (
    ...
    const activated = project.value.end_time / TimeDivision < now || !project.value.is_activated || !isLoggedInNear ? false : true;
    ...
    const progressValue = project.value.end_time / TimeDivision < now || !project.value.is_activated ? 0 : 100 * expiredDuration / projectDuration;
    const estimatedTokenPurchased = (project.value.total_tokens * userDepositedBalance / project.value.total_deposits) / (10 ** decimals);
          ...
          <VStack width='100%' spacing='24px'>
            ...
            <Flex alignItems='center'>
              ...
              <Text fontSize='14px' textAlign='center' padding='2px 0px 0px 8px' color={color.green}>
                Currently Raised: USD ${(project.value.total_deposits / inTokenDecimals) / (project.value.total_tokens / (10 ** decimals))}
              </Text>
            </Flex>
          </VStack>
          <Show above='md'>
            ...
            >
              <VStack spacing='40px' >
                ...
                >
                  <Box width='100%'>
                    ...
                    <Text as='h2' fontSize='18px' textAlign='start' marginTop='10px'>{(project.value.total_deposits / inTokenDecimals).toLocaleString()}</Text>
                  </Box>
                ...
                >
                  <Box width='100%' margin='5px'>
                    ...
                    <Text as='h2' fontSize='18px' textAlign='start' marginTop='10px'>{(project.value.total_tokens / (10 ** decimals)).toLocaleString()}</Text>
                  </Box>
                  <Box>
                    <Flex width='100%' justifyContent='end' flexDirection='column'>
                      <Flex margin='5px' justifyContent='end'>
                ...
                >
                  ...
                  <Flex flexDirection={'column'} padding={4} justifyContent={'center'}>
                    <Input
                      ...
                      border={'none'}
                      type={'number'}
                      max={(userDepositedBalance / inTokenDecimals).toLocaleString()}
                      min={1}
                  ...
                  <Flex flexDirection={'column'} padding={4} justifyContent={'center'}>
                    ...
                    >
                      ...
                      <Flex width='100%' justifyContent='end' flexDirection='column'>
                        ...
                        <Flex justifyContent='end' margin='5px'>
                          <Image src={USDT} />
                          <Text as='h2' fontSize='16px' textAlign='end' marginLeft='15px'>{(userDepositedBalance / inTokenDecimals).toLocaleString()}</Text>
                        </Flex>
                      </Flex>
                    </Flex>
                    <Flex
              ...
              <VStack spacing='40px' >
                <Box width='100%'>
                  <Text as='h1' fontSize='40px' textAlign='end' color={color.progressStatus}>USD$ {(project.value.total_deposits / inTokenDecimals) / (project.value.total_tokens / (10 ** decimals))}</Text>
                ...
                >
                  <Box width='100%'>
                    ...
                    <Text as='h2' fontSize='18px' textAlign='start' marginTop='10px'>{(userDepositedBalance / inTokenDecimals).toLocaleString()}</Text>
                  </Box>
                  <Flex width='100%' justifyContent='end' flexDirection='column'>
                    <Flex margin='5px' justifyContent='end'>
                      <Text fontSize='0.7vw' textAlign='end' marginTop='10px' width='max-content'>{'Tether USD'}</Text>
